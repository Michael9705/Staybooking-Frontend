...
class UploadStay extends React.Component {
    ...
    fileInputRef = React.createRef(); // 创建 ref, 在后面会用到
   
    handleSubmit = async (values) => {
      const formData = new FormData();
      const { files } = this.fileInputRef.current; // 读取的时候 一定要用 .current
      ...
      try {
        ...
        // uploadStay().then(() => {message.success("....")})
      } catch (error) {
        ...
        >
          ...
          <Form.Item name="picture" label="Picture" rules={[{ required: true }]}>
            <input
              ...
              ref={this.fileInputRef}  // 需要用这个 ref 把这个信息抽取出来
              multiple={true}
            />
          </Form.Item>
