...
class RemoveStayButton extends React.Component {
    ...
    render = () => {
        return <Button danger={true} shape="round" type="primary" loading={this.state.loading} onClick={this.handleRemoveStay}>Remove Stay</Button>;
    };
}
...
class MyStays extends React.Component {
    ...
    render() {
      return (
        <List
          loading={this.state.loading} // 转圈圈的效果
          grid={{
            gutter: 16,
            xs: 1,
            sm: 3,
          ...
          dataSource={this.state.data} // 喂数据, 数据是 array of object: [{}, {}, {}, ...]
          renderItem={(item) => ( // 对拉回来的每一个元素，都做一个 List.Item 放在页面上 + 呈现方式
            <List.Item>
              <Card
                key={item.id}
                title={
                ...
                extra={<RemoveStayButton stay={item} onRemoveSuccess={this.loadData} />} //  Remove 按钮放到正确的位置
              >
                {
                  <Carousel
                    dots={false}
...
class HostHomePage extends React.Component {
  render() {
    return (
    /* tab 就是类似书签一样的功能, key 是默认的顺序； 第二个属性：销毁未激活状态的TabPane。切走切回来，底下的 component 会继续 */
